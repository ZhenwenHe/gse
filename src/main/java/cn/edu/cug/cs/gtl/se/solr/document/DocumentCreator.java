...
public class DocumentCreator {
    ...
    private SolrInputDocument documentToSolrInputDocumentForCommonFields(Document d){
        ...
        */
        ...
        Instant resultInstance = Instant.from(zonedDateTime);
        doc.addField("input_date",resultInstance.toString());
        return doc;
    }
    ...
    private List<SolrInputDocument> documentToSolrInputDocument(Document d){
        ...
        if(documentMapper.getMappingType()==DocumentMapper.MappingType.DOC_PRE_RAW_FILE_VALUE ||
            ...
            if(d.getBoundingCount()>0){
                BoundingBox bb = BoundingBoxWrapper.unionOf(d.getBoundingList());
                if(BoundingBoxWrapper.isValid(bb)){

                    doc.addField("bounding",BoundingBoxWrapper.toWKT(bb));
                    //Vertex v = BoundingBoxWrapper.getCenter(bb);
                    //doc.addField("bounding",
                    //        Double.valueOf(v.getOrdinate(0)).toString()+" "+Double.valueOf(v.getOrdinate(1)).toString()
                    //);
                }
            }
        ...
        if(documentMapper.getMappingType()==DocumentMapper.MappingType.DOC_PRE_TEXT_PARAGRAPH_VALUE ||
            ...
            for(String str: d.getContentList()){
                ...
                if(d.getBoundingCount()>0){
                    BoundingBox bb = d.getBounding(i);
                    doc.addField("bounding",BoundingBoxWrapper.toWKT(bb));
                    //Vertex v = BoundingBoxWrapper.getCenter(bb);
                    //doc.addField("bounding",
                    //        Double.valueOf(v.getOrdinate(0)).toString()+" "+Double.valueOf(v.getOrdinate(1)).toString()
